...
if __name__ == '__main__':
    ...
    model_dir = '/root/el_models/'
    model_basename = 'elmodel_d64_h8_fold*.pth'
    ...
    #Test 
    total_preds_EL = torch.Tensor().to(device) 
    for data in tqdm(test_loader): 
        ...
        for model in models: 
            ...
            with torch.no_grad(): 
                ...
                y_EL = torch.sigmoid(y_logits) 

                EL_10M  = EL_10M  + y_EL
                
        ave_EL_10M  = EL_10M  / len(models) 
        total_preds_EL = torch.cat((total_preds_EL, ave_EL_10M), 0) 

    P_EL = total_preds_EL.detach().cpu().numpy().flatten() 

    #Save to local
    column=['pep','hla','EL score']
    results = list()
    for m in range(len(P_EL)):
        results.append([testDataset.data[m][0],testDataset.data[m][1],P_EL[m]])

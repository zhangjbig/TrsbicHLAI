...
def predicting(model, device, loader, criterion, epoch):
...
def LoadDataset(fold_num): 
    ...
    trainDataset = seqData(datapath='/root/trainval_ba_fold10/ba_train_fold{}.csv'.format(fold_num))
    valDataset = seqData(datapath='/root/trainval_ba_fold10/ba_val_fold{}.csv'.format(fold_num))
...
if __name__ == '__main__':
    ...
    for epoch in range(NUM_EPOCHS):    
        ...
        #Train
        train_tru, train_pre, train_cost = train(model, device, train_loader, criterion, optimizer, epoch) 
        ...
        val_tru, val_pre, val_cost = predicting(model, device, valid_loader, criterion, epoch)
